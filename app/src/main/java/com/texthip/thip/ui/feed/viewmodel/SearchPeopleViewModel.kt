...
data class SearchPeopleUiState(
    ...
    val recentSearches: List<String> = listOf("메롱", "메메롱"), // TODO: 실제 최근 검색어 로딩
...
class SearchPeopleViewModel @Inject constructor(
    ...
    fun addRecentSearch(keyword: String) {
        _uiState.update { currentState ->
            val updatedSearches = (listOf(keyword) + currentState.recentSearches)
                .distinct().take(10)
    ...
    fun removeRecentSearch(keyword: String) {
        _uiState.update { currentState ->
            val updatedSearches = currentState.recentSearches.filterNot { it == keyword }
            currentState.copy(recentSearches = updatedSearches)
        }
    }
}
